...
SETTINGS = {"system": "MSE", "vendor": "华三", "device_type": "MSE", "vendor_id": "h3c", "device_id": "mse"}

SEARCHES = {
    "username": [
        {"name": "基于username查看用户是否在线", "fields": [("username", "用户名", "")], "commands": ["display ppp access-user username {username}", "display ip subscriber session username {username}"]},
        {"name": "基于username查看用户上线失败记录", "fields": [("username", "用户名", "")], "commands": ["display aaa online-fail-record username {username}"]},
        {"name": "基于username查看用户下线记录", "fields": [("username", "用户名", "")], "commands": ["display aaa offline-record username {username}"]},
    ],
    "user_ip": [
        {"name": "基于用户IP地址查看用户是否在线", "fields": [("user_ip", "用户IP地址", "")], "commands": ["display ppp access-user ip-address {user_ip}", "display ip subscriber session ip {user_ip}"]},
    ],
    "mac": [
        {"name": "基于用户MAC地址查看用户是否在线", "fields": [("mac", "用户MAC地址", "")], "commands": ["display ppp access-user mac-address {mac}", "display ip subscriber session mac {mac}"]},
        {"name": "基于用户MAC地址查看用户上线失败记录", "fields": [("mac", "用户MAC地址", "")], "commands": ["display aaa online-fail-record mac-address {mac}"]},
        {"name": "基于用户MAC地址查看用户下线记录", "fields": [("mac", "用户MAC地址", "")], "commands": ["display aaa offline-record mac-address {mac}"]},
    ],
}
