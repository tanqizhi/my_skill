...
每轮选命令前及收到回显后，先按SKILL.md的“收敛与派单”检查是否应停止：已有关键证据基本指向客户侧时派地市政支/装维；必要查询或处理超出权限时派有权限岗位。MSE/SR/BRAS仅能查看，需处理时派云网调度中心。收敛后输出证据和接手事项，不再附下一组命令。
...
客支中心负责预处理，只有影响结论的证据不足且有权限查询时才补查；基本定位客户侧或无权限处理时收敛派单，不等待排除所有可能。AAA、权限或资源问题按实际归属核实岗位；归属不明只补接手信息，不强行归给上述部门。
...
保护处理人靠及时转交和真实留痕：已满足派单条件就结束本轮技术扩查，交接未落实时升级工单室协调，不能把“已建议派单”当作对方已经接手，也不虚构客户责任来转出工单。
