...
本Skill已有命令清单就是4A白名单，不另行索要清单。选择命令时直接对应内置条目及模板参数；未列出的命令不返回，不能用通用手册或经验补齐。
...
每轮选命令前及收到回显后，优先判断是否应派单：需要其他部门协调、客户侧可能性较高或必要处理无权限时，先给立即派单建议，不等彻底定位。之后独立判断本岗位是否有可继续做的有效白名单查询，有则列为并行补查；不能把补查完成作为派单前提。MSE/SR/BRAS仍只有查看权限。
...
- 检查OLT/ONU光信息时，按 [质量参照标准](质量参照标准.md) 中对应EPON/GPON制式和方向，对照已测光衰、收光值，记录实测值、单位、阈值和结果；未采集项注明，不强制补齐全部指标。
...
MSE/SR/BRAS只有查看权限，需修复时直接派有权限岗位，不生成变更命令，也不要求操作人找手册后继续操作。其他设备仅在当前岗位已有相应权限时，才按适用手册核对目标、影响、前提、验证、回退并取得本次确认；无权限则派单。

派单、加派及挂单遵循 [组织与工单术语](组织与工单术语.md)。客户侧问题、现场查线、客户侧测试或上门配合指向对应地市分公司政支/装维；OLT层面操作指向对应地市网操；MSE/SR/BRAS/CR/D及传输设备处理指向云网调度中心；服保流转、超时风险和派单协调指向客支中心-工单室。不得只写“接入维护”或“承载专业”。

客支中心负责预处理和及时协同，派单后可主动补查并将结果附回原工单，无需等待对方提出要求。客户侧可能性较高时先派现场核实；归属不明时及时请工单室协调接手岗位，不因定责争议拖延流转。不得无依据指定部门或把疑似写成确定责任。
...
【交接留痕】派单时间、目标岗位、受理状态；未接单/退单/争议/超时风险及工单室协调记录
【挂单核验】不涉及 / 不可挂单 / 证据不足 / 满足条件；涉及时列原因、起止时间/时长、证据和恢复处理条件
【处置状态】建议立即派单 / 已派单且并行补查 / 已派单待协同 / 当前未复现继续跟踪 / 复测且客户确认恢复
【并行补查】本岗位可继续查询的目标/目的、结果及回填情况；无必要或无权限时注明
【恢复验证】处理方反馈、关键查询前后对比、客户原故障业务确认
```

网络侧复测正常但客户未确认时写“网络侧复测正常，业务待确认”，不结案。

保护处理人首先要避免派单不及时：满足条件立即建议派单，之后允许继续有效补查，两者不互相等待。交接未落实时升级工单室协调，不把“已建议派单”当作对方已接手，也不虚构客户责任来转出工单。
